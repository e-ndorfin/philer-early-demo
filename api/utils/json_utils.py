...
def format_initial_form_data(json_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    if json_data.get("Main Applicant"):
        main_applicant = json_data["Main Applicant"][0]  # Get first main applicant
        ...
        if main_applicant.get("Percentage of Ownership") is not None:
            form_data["primary_applicant_ownership_percentage"] = str(main_applicant["Percentage of Ownership"])
    ...
    if json_data.get("Full Address") is not None:
        ...
        address_parts = [part.strip() for part in json_data["Full Address"].split(',')]
...
def save_final_json(form_data: Dict[str, Any]) -> None:
    ...
    final_data["Main Applicant"][0]["Full Name"] = f"{form_data.get('applicant_first_name', '')} {form_data.get('applicant_last_name', '')}".strip() or final_data["Main Applicant"][0].get("Full Name")
    final_data["Main Applicant"][0]["Date of Birth"] = form_data.get('applicant_dob') or final_data["Main Applicant"][0].get("Date of Birth")
    final_data["Main Applicant"][0]["First-time buyer"] = parse_boolean(form_data.get('applicant_first_time_buyer')) if 'applicant_first_time_buyer' in form_data else final_data["Main Applicant"][0].get("First-time buyer")
    final_data["Main Applicant"][0]["Canadian Citizen/PR"] = parse_boolean(form_data.get('applicant_citizenship')) if 'applicant_citizenship' in form_data else final_data["Main Applicant"][0].get("Canadian Citizen/PR")
    final_data["Main Applicant"][0]["Capable of making decisions?"] = parse_boolean(form_data.get('applicant_decision_making')) if 'applicant_decision_making' in form_data else final_data["Main Applicant"][0].get("Capable of making decisions?")
    final_data["Main Applicant"][0]["Marital Status"] = form_data.get('marital_status') or final_data["Main Applicant"][0].get("Marital Status", "Single")
    final_data["Main Applicant"][0]["Percentage of Ownership"] = parse_ownership(form_data.get('primary_applicant_ownership_percentage')) if 'primary_applicant_ownership_percentage' in form_data else final_data["Main Applicant"][0].get("Percentage of Ownership", 1)
    ...
    if 'transaction_type' in form_data:
        final_data["Transaction Type"] = get_transaction_type(form_data.get('transaction_type'))
        
    if 'property_address' in form_data or 'property_postal_code' in form_data:
        current_address = form_data.get('property_address') or final_data.get("Full Address") or ""
        postal_code = form_data.get('property_postal_code')
        
        if postal_code:
            address_parts = [part.strip() for part in current_address.split(',')]
                    ...
                    for pattern in postal_patterns:
                        ...
                        if match:
                            ...
                            if postal_code_index > 0:
                                address_parts[-1] = last_part[:postal_code_index].strip() + " " + postal_code
                            else:
                                address_parts[-1] = postal_code
                            has_postal_in_address = True
                            break
    ...
    if 'property_construction_status' in form_data:
        final_data["Pre Con?"] = parse_boolean(form_data.get('property_construction_status'))
    ...
    if 'mortgage_advisor' in form_data and form_data['mortgage_advisor']:
        ...
        if name or company or lender:
            ...
            final_data["Mortgage Agent"][0]["Email"] = final_data["Mortgage Agent"][0].get("Email") if final_data.get("Mortgage Agent") else None
            final_data["Mortgage Agent"][0]["Phone"] = final_data["Mortgage Agent"][0].get("Phone") if final_data.get("Mortgage Agent") else None
    ...
    if 'real_estate_agent' in form_data and form_data['real_estate_agent']:
        ...
        if name or company:
            ...
            final_data["Realtor"][0]["Email"] = final_data["Realtor"][0].get("Email") if final_data.get("Realtor") else None
            final_data["Realtor"][0]["Phone"] = final_data["Realtor"][0].get("Phone") if final_data.get("Realtor") else None
    ...
    if 'home_insurance' in form_data and form_data['home_insurance'].lower() in ['yes', 'y', 'yeah']:
        if 'home_insurance_details' in form_data and form_data['home_insurance_details']:
            company = extract_insurance_company(form_data['home_insurance_details'])
            agent = extract_insurance_agent(form_data['home_insurance_details'])
            ...
            if company or agent:
                ...
                # Preserve existing fields
                final_data["Insurance Agent"][0]["Email"] = final_data["Insurance Agent"][0].get("Email") if final_data.get("Insurance Agent") else None
                final_data["Insurance Agent"][0]["Phone"] = final_data["Insurance Agent"][0].get("Phone") if final_data.get("Insurance Agent") else None
...
def get_transaction_type(value: str) -> str:
    ...
    value = value.lower()
    if 'buy' in value or 'purchas' in value:
        return "Buy"
    elif 'sell' in value:
        return "Sell"
